...
def markdown_to_html_node(markdown):
    ...
    for block in blocks_list:
        match block_to_block_type(block):
            case "paragraph":
                final_node.children.append(LeafNode("p", block, None, None))
            case "ordered_list":
                final_node.children.append(
                    ParentNode("ol", text_to_children(block, "ol")))
            case "unordered_list":
                final_node.children.append(
                    ParentNode("ul", text_to_children(block, "ul")))
            case "blockquote":
                print(f"INCOMING BLOCK QUOTE: {block}")
                new_string = "\n".join(list(map(lambda line: line.split("> ")[1], block.split("\n"))))
                new_node = LeafNode("blockquote", new_string, None, None)
                final_node.children.append(new_node)
            case "code":
                final_node.children.append(
                    ParentNode("pre", [LeafNode("code", block.split("```")[1], None, None)]))
            case "heading":
                final_node.children.append(LeafNode(f"h{head_level(block)}", lstrip_n(
                    block, "#", head_level(block)).strip(), None, None))

    pprint.pprint(f"FINAL NODE: {final_node.to_html()}")
    return final_node
